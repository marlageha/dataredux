...
def whirc_shiftcalc(obj_id):
    ...
    if len(aobj_idJN2) > 0:
        ...
        #LOOP THROUGH EACH EXPOSURE IN JN2
        for shf_num in range(len(aobj_idJN2)):
            ...
            #FIRST ITERATION, TO GET ROUGH APPROXIMATION
            #x_shift = np.arange(-1000, 1000, 50)
            #y_shift = np.arange(-1000, 1000, 50)
            datai = np.zeros((len(x_shift),len(y_shift)))
            ...
            for x in range(len(x_shift)):
               for y in range(len(y_shift)):
                   ...
                   for shift_line in range(len(shift_im)):
                       ...
                       dist[shift_line] = np.min(dist[shift_line])
                   tot_dist =  sum(dist.values())
                   datai[x, y] = tot_dist #set total distance corresponding to each x,y shift

            #Find x,y shift that yields minimum total distance (for first iteration)
            mins = np.where(datai==datai.min())
            
            mina = [x_shift[mins[0][0]],y_shift[mins[1][0]]]
            #print mina
        
            #-----------------------------Now Repeat--------------------------------#
        
            #SECOND ITERATION, TO GET BETTER APPROXIMATION       
            x_shift2 = np.arange(float(mina[0])-200, float(mina[0])+200, 10)
            y_shift2 = np.arange(float(mina[1])-200, float(mina[1])+200, 10)
            datai = np.zeros((len(x_shift2),len(y_shift2)))
            ...
            mina = [x_shift2[mins[0][0]],y_shift2[mins[1][0]]]
            #print [float(mina[0]),float(mina[1])]
            ...
            mina = [x_shift3[mins[0][0]],y_shift3[mins[1][0]]]
            print [float(mina[0]),float(mina[1])]
    ...
    if len(aobj_idKN2) > 0:
        ...
        #LOOP THROUGH EACH EXPOSURE IN KN1
    ...
    if len(aobj_idJN3) > 0:
        ...
        for shf_num in range(len(aobj_idJN3)):
            ...
            #SECOND ITERATION, TO GET BETTER APPROXIMATION       
            x_shift2 = np.arange(float(mina[0])-100, float(mina[0])+100, 10)
            y_shift2 = np.arange(float(mina[1])-100, float(mina[1])+100, 10)
            datai = np.zeros((len(x_shift2),len(y_shift2)))
    ...
    if len(aobj_idKN3) > 0:
        ...
        #LOOP THROUGH EACH EXPOSURE IN KN3
